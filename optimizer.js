...
function makePrepared(){
  ...
  for(const s of S.samples){
    ...
    if(Number.isFinite(mv))d.rows.push({part:p,machine:machineOf(s,S),board:boardOf(s),value:mv});
  }
...
function evaluate(prep,rows,selected,preferredParts,t){
  ...
  return {
    threshold:t,valid:!!g?.ok&&Number.isFinite(objective),recommended:preferredParts===0||commonParts>=preferredParts,
    selectedParts:selected.length,commonParts,measurements:rows.length,machines,boards,repeats,balancedRows,
    objective,pctStudyVariation:g?.ok?g.pctStudyVariation:NaN,g
  };
}
async function scan(prep,from,to,step,direction,preferredParts){
  ...
  for(let i=0;i<thresholds.length;i++){
    ...
    if(i%refreshEvery===0||i===thresholds.length-1){
      prep.app.progress('Study Optimizer',`Testing threshold ${i+1} / ${thresholds.length}`,12+80*(i+1)/thresholds.length,`${direction} ${fmt(t,4)} · minimizing %Contribution`);await nap(0);
    }
  }
  return results;
}
...
function render(result){
  ...
  if(!valid.length){
    ...
    out.innerHTML=`<div class="opt-warning"><b>No calculable candidate in this range.</b><br>${req}<br><br><b>Best data availability found:</b><br>Machines: ${maxMachines}<br>Boards: ${maxBoards}<br>Common/repeated pins: ${maxCommon}<br>Repeats: ${maxRepeats}</div>`;return;
  }
  ...
  $('optBestGRR').textContent=`${fmt(best.objective,2)}%`;
  $('optBestParts').textContent=best.commonParts.toLocaleString();
  $('optBestNdc').textContent=best.g?.ndc===Infinity?'∞':(best.g?.ndc??'—');
  ...
  out.innerHTML=`<div class="opt-note"><b>Lowest available ${L.name}:</b> ${fmt(best.objective,2)}% at ${result.direction} ${fmt(best.threshold,4)}${unit}.<br><b>Secondary %Study Variation:</b> ${fmt(best.pctStudyVariation,2)}%<br><b>Study mode:</b> ${result.mode==='repeatability'?'Same-board Repeatability':result.mode==='process'?'Production Process Variation':'Crossed GR&R'}${result.mode!=='crossed'?` · Machine ${esc(result.target)}`:''}<br><b>Coverage:</b> ${coverage}${modelNote}<br><br><b>Measurement:</b> ${esc(result.measurementMetric.fullLabel)}<br><b>Optimized filter:</b> Rule ${result.ruleIndex+1} · ${esc(result.filterMetric.fullLabel)}</div>${boundary}<div class="opt-interpret"><b>How to read this result</b><br>${esc(contributionInterpretation(result.mode,best.objective))}<br><br>The optimizer ranks thresholds by <b>%Contribution (of VarComp)</b>, the same variance-component KPI shown in Minitab. %Study Variation remains available as a secondary diagnostic.</div><div class="opt-table-wrap"><table class="opt-table"><thead><tr><th>Rank</th><th>Threshold</th><th>%Contribution</th><th>%Study Var</th><th>Common/repeated pins</th><th>Measurements</th><th>Machines</th><th>Boards</th><th>Repeats</th></tr></thead><tbody>${top.map((r,i)=>`<tr class="${i===0?'best':''}"><td>${i+1}</td><td>${result.direction} ${fmt(r.threshold,4)}${unit}</td><td>${fmt(r.objective,2)}%</td><td>${fmt(r.pctStudyVariation,2)}%</td><td>${r.commonParts}</td><td>${r.measurements}</td><td>${r.machines}</td><td>${r.boards}</td><td>${r.repeats}</td></tr>`).join('')}</tbody></table></div><div class="opt-actions"><button id="applyBestThreshold" class="primary">Apply lowest-%Contribution threshold & analyze</button><button id="downloadOptCsv" class="ghost">Export optimizer CSV</button></div>`;
  $('applyBestThreshold').onclick=async()=>{if(result.app.applyRuleThreshold(result.ruleIndex,result.direction,best.threshold)){result.app.toast(`Applied Rule ${result.ruleIndex+1}: ${result.direction} ${best.threshold}`);if(window.XGRRStudy?.analyzeStudy)await window.XGRRStudy.analyzeStudy();else await result.app.analyze()}};
  $('downloadOptCsv').onclick=()=>downloadCsv(valid,result);
  setTimeout(()=>window.XGRRUI?.enhanceOptimizerMobile?.(),0);
}
function downloadCsv(rows,result){
  const L=labels(result.mode),cols=['Threshold','Direction','Study_Mode','Objective_Name','Contribution_Percent','Study_Variation_Percent','Common_Parts','Selected_Parts','Measurements','Machines','Boards','Repeats','NDC','Interaction_P_Value','Interaction_Included','Filter_Metric','Measurement_Metric'],lines=[cols.join(',')],q=v=>`"${String(v??'').replaceAll('"','""')}"`;
  for(const r of [...rows].sort((a,b)=>a.threshold-b.threshold))lines.push([r.threshold,result.direction,result.mode,q(L.name),r.objective,r.pctStudyVariation,r.commonParts,r.selectedParts,r.measurements,r.machines,r.boards,r.repeats,r.g?.ndc===Infinity?'Infinity':(r.g?.ndc??''),r.g?.interactionPValue??'',r.g?.interactionIncluded??'',q(result.filterMetric.fullLabel),q(result.measurementMetric.fullLabel)].join(','));
...
async function optimize(){
  const app=A();try{
    resetOutput();
    const prep=makePrepared(),from=+$('optFrom').value,to=+$('optTo').value,step=+$('optStep').value,preferredParts=Math.max(0,Math.floor(+$('optMinParts').value||0)),direction=$('optDirection').value;
    ...
    app.progress('Study Optimizer','Preparing data matrix…',5,`${filterMetric?.fullLabel||filterKey} → ${measurementMetric?.fullLabel||prep.measurementMetric}`);await nap(10);
    const results=await scan(prep,from,to,step,direction,preferredParts);
    app.progress('Study Optimizer','Ranking thresholds by %Contribution (of VarComp)…',96);await nap(10);
    const result={...prep,results,from,to,step,direction,preferredParts,filterMetric,measurementMetric};render(result);
    app.progress('Done','Lowest %Contribution threshold search ready.',100);await nap(80);app.hideProgress();
...
function sync(){
  const app=A(),rules=app?.getCurrentRules?.()||[],opt=$('optRule'),btn=$('optimizeBtn');if(!opt||!btn)return;
  const old=+opt.value;if(!rules.length){opt.innerHTML='<option>No filter rules</option>';opt.disabled=btn.disabled=true;resetOutput();return}
