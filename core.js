...
function calculateProcessVariation(rows){
 rows=rows.filter(r=>r&&Number.isFinite(+r.value)&&r.part!=null&&r.board!=null&&String(r.board)!=='').map(r=>({part:String(r.part),board:String(r.board),value:+r.value}));
 const parts=[...new Set(rows.map(r=>r.part))],boards=[...new Set(rows.map(r=>r.board))];if(parts.length<2||boards.length<2)return {ok:false,reason:'Need at least 2 physical boards and 2 common pins/parts.'};
 const cell=new Map();for(const r of rows){const k=r.part+'\0'+r.board;if(!cell.has(k))cell.set(k,[]);cell.get(k).push(r.value)}
 const common=parts.filter(p=>boards.every(b=>(cell.get(p+'\0'+b)||[]).length>=1));if(common.length<2)return {ok:false,reason:'Need at least 2 pins/parts measured on every physical board.'};
 const data=[];for(const p of common)for(const b of boards){const v=cell.get(p+'\0'+b);data.push({part:p,board:b,value:mean(v)})}
 const P=common.length,B=boards.length,grand=mean(data.map(r=>r.value)),partMean=new Map(common.map(p=>[p,mean(data.filter(r=>r.part===p).map(r=>r.value))])),boardMean=new Map(boards.map(b=>[b,mean(data.filter(r=>r.board===b).map(r=>r.value))]));let ssP=0,ssB=0,ssE=0;
 for(const p of common)ssP+=B*Math.pow(partMean.get(p)-grand,2);for(const b of boards)ssB+=P*Math.pow(boardMean.get(b)-grand,2);for(const r of data)ssE+=Math.pow(r.value-partMean.get(r.part)-boardMean.get(r.board)+grand,2);
 const dfP=P-1,dfB=B-1,dfE=(P-1)*(B-1),msP=ssP/dfP,msB=ssB/dfB,msE=ssE/dfE,varResidual=Math.max(msE,0),varBoard=Math.max((msB-msE)/P,0),varPart=Math.max((msP-msE)/B,0),varProcess=varBoard+varResidual,varTotal=varProcess+varPart,sProcess=Math.sqrt(varProcess),sBoard=Math.sqrt(varBoard),sResidual=Math.sqrt(varResidual),sPart=Math.sqrt(varPart),sTotal=Math.sqrt(varTotal),pctProcess=sTotal>EPS?100*sProcess/sTotal:NaN,pctContribution=varTotal>EPS?100*varProcess/varTotal:NaN,cv=Math.abs(grand)>EPS?100*sProcess/Math.abs(grand):NaN;
 const boardStats=boards.map(b=>({board:b,...summarize(data.filter(r=>r.board===b).map(r=>r.value))})).sort((a,b)=>b.mean-a.mean);
 return {ok:true,method:'production_process',parts:P,boards:B,observations:data.length,grandMean:grand,variance:{boardToBoard:varBoard,residual:varResidual,process:varProcess,partToPart:varPart,total:varTotal},sd:{boardToBoard:sBoard,residual:sResidual,process:sProcess,partToPart:sPart,total:sTotal},studyVariation:{process:6*sProcess,boardToBoard:6*sBoard,residual:6*sResidual,total:6*sTotal},pctStudyVariation:pctProcess,pctContribution,processCV:cv,commonParts:P,boardStats,anova:{ssPart:ssP,ssBoard:ssB,ssResidual:ssE,dfPart:dfP,dfBoard:dfB,dfResidual:dfE,msPart:msP,msBoard:msB,msResidual:msE}};
}
global.XGRRCore={mean,sd,summarize,logicKey,logicInfo,logicLabel,metricKey,metricInfo,metricLabel,metricFullLabel,compare,calculateCrossedANOVA,calculateSingleMachineRepeatability,calculateProcessVariation};
})(window);